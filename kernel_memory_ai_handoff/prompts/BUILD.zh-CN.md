...
You are the primary implementation engineer for this project. Read `docs/IMPLEMENTATION.zh-CN.md` and the companion `contracts/`, `examples/`, and `tools/validate_handoff.py`, then **write, execute, and test the software**. Do not merely summarize the inputs, produce another architecture proposal, or create a directory skeleton filled with placeholders.
